...
export default function EarningsHistoryScreen() {
  ...
  const { totalEarnings, viewEarnings, giftEarnings, ledgers } = useWalletStore();
    ...
    <View className="flex-1 bg-[#12081E] pt-14">
      ...
      <View className="bg-[#1A0E2C] border border-white/5 rounded-3xl p-6 gap-6">
          <View className="flex-row items-center justify-between">
            <View>
              <Text className="text-neutral-grey text-xs font-medium">Total Earnings</Text>
              <Text className="text-white text-3xl font-black mt-1">{formatINR(totalEarnings)}</Text>
            </View>
          ...
          <View className="flex-row gap-4 border-t border-white/5 pt-4">
            <View className="flex-1">
              ...
              <Text className="text-white font-bold mt-1">{formatINR(viewEarnings)}</Text>
            </View>
            <View className="flex-1 border-l border-white/5 pl-4">
              <Text className="text-neutral-grey text-[10px] uppercase">Gifts</Text>
              <Text className="text-white font-bold mt-1">{formatINR(giftEarnings)}</Text>
            </View>
          </View>
        </View>
            ...
            {ledgers.length === 0 ? (
              ...
              ledgers.map((l) => (
                <HistoryRow
                  ...
                  amount={formatINR(l.credit)}
                  source={l.description || l.source.replace('_', ' ')}
                />
              ))
            )}
          </View>
